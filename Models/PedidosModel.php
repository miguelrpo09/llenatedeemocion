...

	{
		...
		public function selectPedidos($idpersona = null){
			...
			$sql = "SELECT  p.idpedido,
							...
							CONCAT (ps.nombres,' ',ps.apellidos) AS nombrecompleto
					FROM pedido p 
					INNER JOIN persona ps ON p.personaid = ps.idpersona 
					INNER JOIN detalle_pedido dp ON p.idpedido = dp.pedidoid
					INNER JOIN producto pr ON dp.productoid = pr.idproducto 
					INNER JOIN tipopago tp ON p.tipopagoid = tp.idtipopago $where ";
